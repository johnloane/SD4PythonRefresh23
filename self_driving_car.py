...
def random_augment(image_to_augment, steering_angle):
    ...
    if np.random.rand() < 0.5:
        augment_image = zoom(augment_image).astype("uint8")
    if np.random.rand() < 0.5:
        augment_image = pan(augment_image).astype("uint8")
    # if np.random.rand() < 0.5:
...
history = model.fit(batch_generator(X_train, y_train, 100, 1), epochs=20, validation_data=batch_generator(X_valid, y_valid, 100, 0), validation_steps=200, verbose=1, shuffle=1)
